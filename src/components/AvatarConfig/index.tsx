  ...
  const selectedAvatar = useMemo(() => {
    ...
    if (!avatar) {
      return {
        ...
        avatarId: null,
        role: "Custom AI",
        description: "Manually entered avatar configuration."
      };
    } else {
      return {
        isCustom: false,
  ...
  return (
    <div className="relative flex flex-col gap-4 w-full py-4 max-h-full overflow-y-auto">
      ...
      {!selectedAvatar.isCustom && (
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 rounded-2xl text-white shadow-xl mb-2">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-bold uppercase tracking-wider bg-white/20 px-2 py-1 rounded">
              {selectedAvatar.role}
            </span>
          </div>
          <h2 className="text-2xl font-bold mb-2">{selectedAvatar.name}</h2>
